...
{
	//static int rssi = 0;
	static int rssi = -80;  //³õÖµÐÅºÅ×îÈõ
	static int grssi_prev = -80;
...
int init_ap_profile_from_string(char *param_str, struct ap_profile *ap_cfg)
{
	...
	ret = get_parmeter_from_string(&str_ptr, "SSID=", PTYPE_STRING, ap_cfg->ssid, SSID_LEN-1);
	ret |= get_parmeter_from_string(&str_ptr, "SEC=", PTYPE_STRING,  ap_cfg->sec, SEC_LEN-1);
	ret |= get_parmeter_from_string(&str_ptr, "KEY=", PTYPE_STRING,  ap_cfg->key, KEY_LEN-1);
...
)
{
	...
	case IW_AUTH_KEY_MGMT:
		if ((error = dev_wlc_intvar_get(dev, "wpa_auth", &val))) {
			WL_ERROR(("Failed to get 'wpa_auth'iovar, error=%d\n", error));
			return error;
		}
	...
	char *ext)
{
	...
	if ((wrqu->data.length >= 3) && (wrqu->data.length < sizeof(extra))) {
		...
		if (get_parmeter_from_string(&str_ptr, "FW_PATH=", PTYPE_STRING, fwstr, 255) != 0) {
			strcpy(fwstr, firmware_softap_path);
			WL_SOFTAP(("SOFTAP FIRMWARE PATH=%s\n", fwstr));
			//WL_ERROR(("Error: extracting FW_PATH='' string\n"));
			//goto exit_proc;
		} else {
			WL_SOFTAP(("EXTERNAL SOFTAP FIRMWARE PATH=%s\n", fwstr));
        }
